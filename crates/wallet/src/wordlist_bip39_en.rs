//! Official **BIP-39 English** mnemonic word list (2048 words).
//!
//! Vendored verbatim, in order, from `bip-0039/english.txt` in the BIP repository
//! (bitcoin/bips).  Order is consensus-critical: a word's index *is* its 11-bit value, so the
//! list must never be re-sorted, deduplicated or "corrected".  One transposed pair here would
//! silently mint unspendable addresses for every phrase that touches either word, which is why
//! [`crate::bip39`] gates this file against the structural properties BIP-39 itself specifies
//! (2048 entries, lowercase ASCII, strictly sorted, unique, unique 4-character prefixes) plus a
//! pinned SHA-256 of the canonical file.
//!
//! **This is not the Monero list.**  [`crate::wordlist_en`] is Monero's *1626*-word CryptoNote
//! list, used by [`crate::mnemonic`] for the 25-word spend-key phrase.  The two lists share a
//! language and nothing else - different length, different index semantics, different checksum.
//! Passing one where the other is expected produces a wrong key, not an error.

/// The 2048 BIP-39 English mnemonic words, in canonical index order (index = 11-bit value).
///
/// `static` (not `const`) so the large array is materialised once rather than copied at each use.
