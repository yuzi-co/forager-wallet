...
2. Re-derive the same address from the same secret, in software that is not this tool:
   - single-key: any standard WIF importer, or `forager-wallet inspect <secret-hex> --coin <ticker>`
     as a cheap first check;
   - `--hd`: any standard BIP39/BIP44 wallet, from the same 24-word mnemonic.
